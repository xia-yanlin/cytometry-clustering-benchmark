...
def main() -> None:
    ...
    columns = ['dataset', 'configuration', 'grid_side', 'rlen',
               'ari_mean', 'macro_f1_mean', 'median_bmu_distance_mean']
    ...
    for dataset, group in data.groupby('dataset', sort=True):
        ...
        for objective, field, ascending in [
            ...
            selected = group.sort_values([field,'grid_side','rlen'],
                ascending=[ascending,True,True],kind='stable').iloc[0]
            result.append({'dataset':dataset,'selection_objective':objective,
                'configuration':selected.configuration,'grid_side':int(selected.grid_side),
                'rlen':int(selected.rlen),'mean_ARI':selected.ari_mean,
                'mean_macro_F1':selected.macro_f1_mean,
                'mean_median_BMU_distance':selected.median_bmu_distance_mean,
                'candidate_configurations':9,'seeds_per_configuration':5,
                'held_out_evaluation':False})
    args.out.mkdir(parents=True, exist_ok=True)
    pd.DataFrame(result).to_csv(args.out/'selection_objective_sensitivity.csv',index=False)
