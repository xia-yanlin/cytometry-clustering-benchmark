...
def rare_panel(ax, data, dataset, letter):
    ...
    groups = [
        ...
        ("GMM", "K=10", ORANGE, "s"),
        ("GMM", "K=40", GREEN, "^"),
        ("official_Xshift", "neighbor_K=20", DARK, "D"),
    ]
    for j, (method, regime, color, marker) in enumerate(groups):
        ...
        for i, metric in enumerate(metrics):
            ...
            if len(values) > 1:
                ax.vlines(xpos[i], values.min(), values.max(), color=color, alpha=0.7, lw=1.3)
            ax.scatter(
                xpos[i],
                values.mean(),
                s=24 if j < 3 else 37,
    ...
    ax.grid(axis="y", color="#E1E5E8", linewidth=0.5)
...
def figure1():
    ...
    ax.set(
        xlabel="Background clusters in prespecified simulation",
        ylabel="Metric value",
        ylim=(-0.04, 1.05),
    )
    ax.grid(axis="y", color="#E1E5E8", linewidth=0.5)
    ...
    handles = [
        Line2D([0], [0], color=col, marker=mark, linestyle="none", markersize=5, label=txt)
        for txt, col, mark in [
            ("GMM K=2", BLUE, "o"),
            ("GMM K=10", ORANGE, "s"),
            ("GMM K=40", GREEN, "^"),
    ...
    fig.legend(
        ...
        frameon=False,
        fontsize=7,
        title="B–C: method and parameter regime",
    )

    ax = axes[1, 1]
    ax.scatter(c.recall, c.precision, s=21, color=PURPLE, alpha=0.85)
    for _, row in c.iterrows():
        if row.f1 < 0.3 or row.recall < 0.25:
    ...
    ax.plot([0, 1], [0, 1], ls=":", color="#B8BFC4", lw=0.7)
    ax.set(xlabel="Recall", ylabel="Precision", xlim=(-0.04, 1.04), ylim=(-0.04, 1.04))
    ax.grid(color="#E8ECEF", linewidth=0.45)
    label(ax, "D", "Levine 32dim: 14 reference populations")
    save(
        fig,
        "Figure1_reference_metrics",
        [
            "figure1a_reference_simulation.csv",
            "figure1b_rare_run_points.csv",
...
def figure2():
    ...
    axa.set(ylabel="Metric on evaluable events", ylim=(0, 1), xlim=(-0.15, 1.15))
    axa.legend(frameon=False, loc="lower left")
    axa.grid(axis="y", color="#E1E5E8", linewidth=0.5)
    ...
    for metric, color, marker, name in [
        ("precision", BLUE, "o", "Precision"),
        ("recall", ORANGE, "s", "Recall"),
        ("f1", GREEN, "^", "F1"),
    ]:
    ...
    axc.grid(axis="y", color="#E1E5E8", linewidth=0.5)
    ...
    regime_map = {
        ...
        "official_fixed_ktrue": "K=true*",
        "official_fixed_k40": "Fixed K=40",
    }
    for ax, ds in [(axb1, "Levine_32dim"), (axb2, "Samusik_01")]:
        sub = b[b.dataset.eq(ds)]
        for j, regime in enumerate(regime_map):
            rows = sub[sub.regime.eq(regime)]
            if len(rows) != 30:
                raise ValueError((ds, regime, len(rows)))
            for metric, off, color, marker in [
                ("ari", -0.11, BLUE, "o"),
                ("macro_f1", 0.11, ORANGE, "s"),
            ]:
                vals = rows[metric].to_numpy(float)
                xx = j + off + np.linspace(-0.045, 0.045, len(vals))
                ax.scatter(xx, vals, color=color, alpha=0.30, s=8, marker=marker, linewidth=0)
                ax.scatter(
                    j + off,
                    vals.mean(),
                    color=color,
        ...
        ax.set(xlim=(-0.42, 2.42), ylim=(0, 1), ylabel="ARI / Macro F1")
        ax.grid(axis="y", color="#E1E5E8", linewidth=0.5)
        panel = "B1" if ds == "Levine_32dim" else "B2"
        label(ax, panel, ds.replace("_", " "))
    axb2.legend(
        handles=[
            Line2D([0], [0], marker="o", color=BLUE, ls="none", label="ARI"),
...
def run_strip(ax, data, letter, title):
    ...
    for j, ds in enumerate(datasets):
        ...
        for metric, off, color, marker in [
            ("ari", -0.10, BLUE, "o"),
            ("macro_f1", 0.10, ORANGE, "s"),
        ]:
            vals = rows[metric].to_numpy(float)
            xx = j + off + np.linspace(-0.045, 0.045, len(vals))
            ax.scatter(xx, vals, s=10, alpha=0.47, color=color, marker=marker, linewidth=0)
            ax.scatter(
                j + off,
                vals.mean(),
                s=42,
    ...
    ax.set(xlim=(-0.45, len(datasets) - 0.55), ylim=(0.4, 1), ylabel="ARI / Macro F1")
    ax.grid(axis="y", color="#E1E5E8", linewidth=0.5)
...
def figure3():
    ...
    handles = [
        Line2D([0], [0], color=BLUE, marker="o", ls="none", label="ARI"),
        Line2D([0], [0], color=ORANGE, marker="s", ls="none", label="Macro F1"),
    ]
    axa.legend(handles=handles, frameon=False, ncol=2, loc="lower left")
    short = {
        "FlowSOM_R_2.18.0_full_event": "R FlowSOM",
        "PhenoGraph_v1.5.7_default_Louvain": "PhenoGraph",
    ...
    axc.set(xlim=(0.68, 1.015), xlabel="Within-method pairwise partition ARI", ylabel="")
    axc.grid(axis="x", color="#E1E5E8", linewidth=0.5)
    label(axc, "C", "Mean diamond; line = min–max across 435 dependent pairs")
    save(
        fig,
        "Figure3_repeat_stability",
        [
            "figure3a_flowsom_r_run_points.csv",
            "figure3b_phenograph_run_points.csv",
