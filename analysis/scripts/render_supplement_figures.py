...
def grid(ax, axis="x"):
    ax.grid(axis=axis, color="#E2E7EA", lw=.5)
...
def s1():
    ...
    y = np.arange(len(d))
    for metric, off, color, marker in [("precision",-.18,BLUE,"o"),("recall",0,ORANGE,"s"),("f1",.18,GREEN,"^")]:
    ...
    ax.set(xlim=(-.025,1.025), xlabel="Per-reference-population metric")
    ax.legend(ncol=3, frameon=False, loc="lower right")
    grid(ax)
    ax.set_title("Figure S1  Levine 32dim: all 14 reference populations", loc="left", fontweight="bold")
    save(fig,"FigureS1_population_metrics",["xshift_levine32_population.csv"],
...
def s2():
    ...
    for ax,(ds,sub) in zip(axs,d.groupby("dataset",sort=False)):
        for grid_side,color in [(10,BLUE),(15,ORANGE),(20,GREEN)]:
            ...
            for rlen,marker in [(10,"o"),(20,"s"),(30,"^")]:
                ...
                ax.scatter(one.ari_mean,one.macro_f1_mean,color=color,s=42,marker=marker,
                           label=f"{grid_side}×{grid_side}" if rlen==10 else None)
        frontier=sub[sub.pareto_front_mean.astype(str).str.lower().eq("true")]
        ax.scatter(frontier.ari_mean,frontier.macro_f1_mean,s=85,facecolors="none",
                   edgecolors=DARK,linewidth=1.1,label="3-objective Pareto")
        ax.set(xlabel="Mean ARI",ylabel="Mean Macro F1",xlim=(.45,.90),ylim=(.58,.79))
        ax.set_title(ds.replace("_"," "),loc="left")
        grid(ax,"both")
    grid_handles=[Line2D([0],[0],color=c,marker="o",ls="none",label=f"{n}×{n}") for n,c in [(10,BLUE),(15,ORANGE),(20,GREEN)]]
    rlen_handles=[Line2D([0],[0],color=DARK,marker=m,ls="none",label=f"rlen {r}") for r,m in [(10,"o"),(20,"s"),(30,"^")]]
    pareto_handle=Line2D([0],[0],color=DARK,marker="o",markerfacecolor="none",ls="none",label="3-objective Pareto")
    axs[1].legend(handles=grid_handles+rlen_handles+[pareto_handle],frameon=False,loc="lower left",fontsize=5.8,ncol=2)
    fig.suptitle("Figure S2  FlowSOM grid × rlen: external metrics and Pareto status",fontweight="bold",x=.01,ha="left")
    save(fig,"FigureS2_flowsom_pareto",["flowsom_pareto.csv"],
...
def s3():
    ...
    for ax,ds in zip(axes[:5],datasets):
        ...
        ax.set(xlabel="X-shift neighbor K",ylabel="Target F1" if "rare" in ds else "Macro F1",ylim=(0,1))
        ax.set_xticks(sub.knn_k)
        ax.set_title(ds.replace("_"," "),loc="left")
        grid(ax,"y")
    for ax,ds in zip(axes[5:7],["Levine_32dim","Samusik_01"]):
        sub=f[(f.dataset.eq(ds)) & f.metric.eq("macro_f1")]
        if sub.empty: raise ValueError((ds,"macro_f1"))
        ...
        grid(ax,"y")
    for ax,ds in zip(axes[7:9],["Nilsson_rare","Mosmann_rare"]):
        sub=g[(g.dataset.eq(ds)) & g.metric.eq("target_f1")].sort_values("k")
        ax.errorbar(sub.k,sub["mean"],yerr=[sub["mean"]-sub["min"],sub["max"]-sub["mean"]],
                    fmt="o-",capsize=2,color=GREEN,lw=1.2,ms=4)
        ax.set_xticks(sub.k)
        ax.set(xlabel="GMM components K",ylabel="Target F1",ylim=(0,1))
        ax.set_title("GMM | "+ds.replace("_"," "),loc="left")
        grid(ax,"y")
    fig.suptitle("Figure S3  Specified parameter regimes (different K semantics)",x=.01,ha="left",fontweight="bold")
    save(fig,"FigureS3_parameter_regimes",["xshift_k_sensitivity.csv","flowsom_auto_multiclass.csv","gmm_rare_regimes.csv"],
...
def s4():
    ...
    ax=fig.add_subplot(gs[0,:]); axb=fig.add_subplot(gs[1,0]); axc=fig.add_subplot(gs[1,1])
    ...
    method_names={"Leiden_fixed_PhenoGraph_graph":"PhenoGraph–Leiden",
                  "PhenoGraph_default_Louvain":"PhenoGraph–Louvain"}
    ax.set_yticks(y,[f"{method_names.get(str(r.algorithm), str(r.algorithm).replace('_',' '))} | {str(r.dataset).replace('_',' ')}" for _,r in d.iterrows()])
    ax.set(xlabel="Run ARI: mean diamond, min–max line",xlim=(-.04,1.04))
    ...
    axb.barh([1,0],[rejected,len(t)-rejected],color=[ORANGE,BLUE],height=.6)
    ...
    for yy,n in [(1,rejected),(0,len(t)-rejected)]:
        axb.text(n+.5,yy,str(n),va="center",fontsize=8)
    axb.set_title("B  Global Holm: 40 endpoints",loc="left",fontweight="bold")
    grid(axb,"x")
    axc.plot(p.mean_separation_sd_units,p.independent_power,"o-",color=GREEN,label="Independent implementation")
    axc.plot(p.mean_separation_sd_units,p.original_power,"s--",color=PURPLE,label="Original implementation")
    axc.set(xlabel="Mixture mean separation (SD units)",ylabel="Estimated rejection probability",ylim=(0,1))
    axc.set_title("C  n=30 power simulation",loc="left",fontweight="bold")
    axc.legend(frameon=False,fontsize=6)
    grid(axc,"y")
    fig.suptitle("Figure S4  Stability diagnostics and test power",x=.01,ha="left",fontweight="bold")
    save(fig,"FigureS4_stability_diagnostics",["stability_40cells.csv","stability_40tests.csv","multimodality_power.csv"],
...
def s5():
    ...
    for row,ds in enumerate(["Levine_32dim","Samusik_01"]):
        for col,(metric,title) in enumerate([("delta_ari_from_baseline","Δ ARI"),("delta_macro_f1_from_baseline","Δ Macro F1")]):
            ax=axs[row,col]
            for method,color,marker in [("X-shift",BLUE,"o"),("FlowSOM",ORANGE,"s")]:
                for kind,ls in [("boundary_flip","-"),("boundary_exclude","--")]:
                    sub=d[d.dataset.eq(ds)&d.algorithm.eq(method)&d.kind.eq(kind)].sort_values("nominal_fraction")
                    ax.plot(sub.nominal_fraction*100,sub[metric],color=color,marker=marker,ls=ls,lw=1.2,ms=4)
            ax.axhline(0,color="#7D858A",lw=.7)
            ax.set(xlabel="Nominal boundary fraction (%)",ylabel=title,xticks=[1,5,10])
            ax.set_title(ds.replace("_"," ")+" | "+title,loc="left")
            grid(ax,"y")
    handles=[Line2D([0],[0],color=BLUE,marker="o",label="X-shift"),
    ...
    fig.suptitle("Figure S5  Simulated boundary-label perturbations",x=.01,ha="left",fontweight="bold")
    save(fig,"FigureS5_label_perturbation",["label_perturbation.csv"],
...
def s6():
    ...
    for ax,mean,sd,color,title in [(axs[0],"ari_mean_across_chain_mean","ari_mean_across_chain_sd",BLUE,"ARI"),
                                    (axs[1],"macro_f1_mean_across_chain_mean","macro_f1_mean_across_chain_sd",ORANGE,"Macro F1")]:
        multi=d[d.n_marker_chains_contributing.gt(1)]
        ax.errorbar(multi.n_markers,multi[mean],yerr=multi[sd],fmt="o-",color=color,capsize=3,lw=1.3,ms=4)
        single=d[d.n_marker_chains_contributing.eq(1)]
        ax.scatter(single.n_markers,single[mean],color=DARK,marker="D",s=34,label="Shared full-32 reference")
        ax.set(xlabel="Markers retained in nested chain",ylabel=title,ylim=(0,1),xticks=d.n_markers)
        grid(ax,"y")
        ax.set_title(title,loc="left",fontweight="bold")
    axs[1].legend(frameon=False,loc="lower right",fontsize=6.2)
    fig.suptitle("Figure S6  Levine 32dim nested marker sensitivity",x=.01,ha="left",fontweight="bold")
    save(fig,"FigureS6_marker_dimension",["marker_dimension.csv"],
