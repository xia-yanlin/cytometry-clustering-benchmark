...
DATASETS = {
    "Levine_32dim": {"events": 265627, "evaluable": 104184, "populations": 14, "sha": "bb1f9cb63377f701795f116e557f8ede05141b6aca2a0270177c7b1eb4c807e4"},
    "Samusik_01": {"events": 86864, "evaluable": 53173, "populations": 24, "sha": "86dd329dedec30b86ab0cc845923620393a36f16bd07d8f5d87ba4dbb3511565"},
}
...
def ranked_indices(labels: np.ndarray, scores: np.ndarray, eligible: np.ndarray, fraction: float) -> np.ndarray:
    chosen = []
...
def random_indices(labels: np.ndarray, eligible: np.ndarray, target_counts: dict[str, int], seed: int) -> np.ndarray:
...
def local_uncertainty(matrix: np.ndarray, labels: np.ndarray) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    for row in range(len(labels)):
        values, counts = np.unique(neighbor_labels[row][neighbor_labels[row] != labels[row]], return_counts=True)
        if len(values):
...
def main() -> int:
    ...
    prediction_runs = {
        "Levine_32dim": {"X-shift": args.levine_xshift.resolve(), "FlowSOM": args.levine_flowsom.resolve()},
        "Samusik_01": {"X-shift": args.samusik_xshift.resolve(), "FlowSOM": args.samusik_flowsom.resolve()},
    }
    checks: list[dict] = []
    results: list[dict] = []
    ...
    for dataset, contract in DATASETS.items():
        ...
        check(dataset, "data_contract", sha256(source) == contract["sha"] and matrix.shape[0] == contract["events"] and len(labels) == contract["evaluable"] and len(np.unique(labels)) == contract["populations"], f"source={sha256(source)}; matrix={matrix.shape}; evaluable={len(labels)}; populations={len(np.unique(labels))}")
        ...
        for algorithm, run in prediction_runs[dataset].items():
            ...
            if algorithm == "X-shift":
                ...
                full_predictions = np.load(run / "metacluster_labels_all_events.npy", allow_pickle=False)
            predictions[algorithm] = full_predictions[evaluable]
            check(dataset, f"{algorithm}_prediction_contract", manifest_ok and full_predictions.shape == (contract["events"],), f"run={run}; manifest={manifest_ok}; shape={full_predictions.shape}")

        neighbors, disagreement, alternative, has_alternative = local_uncertainty(eval_matrix, labels)
        ...
        neighbor_ok = neighbors.shape == (len(labels), 15) and not np.any(neighbors == np.arange(len(labels))[:, None]) and disagreement.min() >= 0 and disagreement.max() <= 1
        check(dataset, "local_uncertainty_contract", neighbor_ok, f"neighbors={neighbors.shape}; disagreement={float(disagreement.min())}-{float(disagreement.max())}; alternatives={int(has_alternative.sum())}")

        scenarios: list[tuple[str, str, float, int | None, np.ndarray, np.ndarray]] = []
        scenarios.append(("baseline", "unchanged", 0.0, None, labels.copy(), np.ones(len(labels), dtype=bool)))
        ...
        for fraction in (0.01, 0.05, 0.10):
            ...
            scenarios.append((f"boundary_flip_{int(fraction*100):02d}pct", "boundary_flip", fraction, None, flipped, np.ones(len(labels), dtype=bool)))
            excluded = ranked_indices(labels, disagreement, np.ones(len(labels), dtype=bool), fraction)
            keep = np.ones(len(labels), dtype=bool)
            keep[excluded] = False
            scenarios.append((f"boundary_exclude_{int(fraction*100):02d}pct", "boundary_exclude", fraction, None, labels.copy(), keep))
        counts5 = {str(label): int(np.sum(labels[boundary_sets[0.05]] == label)) for label in np.unique(labels)}
        for seed in range(30):
            chosen = random_indices(labels, has_alternative, counts5, seed)
            flipped = labels.copy()
            flipped[chosen] = alternative[chosen]
            scenarios.append((f"random_control_05pct_seed{seed:02d}", "random_control_flip", 0.05, seed, flipped, np.ones(len(labels), dtype=bool)))
        ...
        for name, kind, fraction, seed, scenario_labels, keep in scenarios:
            ...
            check(dataset, f"scenario_{name}", len(scenario_labels) == len(labels) and keep.shape == (len(labels),) and (kind != "boundary_flip" or changed > 0) and (kind != "boundary_exclude" or excluded_count > 0), f"changed={changed}; excluded={excluded_count}; retained={int(keep.sum())}")
            scenario_rows.append({"scenario": name, "kind": kind, "nominal_fraction": fraction, "seed": seed, "changed_labels": changed, "excluded_events": excluded_count, "retained_events": int(keep.sum()), "labels_sha256": sha256(scenario_path), "mask_sha256": sha256(mask_path)})
            for algorithm, predicted in predictions.items():
                metrics, population, _, _, _ = evaluate_multiclass(scenario_labels[keep], predicted[keep], predicted[keep])
                results.append({"dataset": dataset, "algorithm": algorithm, "scenario": name, "kind": kind, "nominal_fraction": fraction, "seed": seed, "changed_labels": changed, "excluded_events": excluded_count, **metrics})
        ...
        write_json(dataset_dir / "source_links.json", {
            "data": str(source), "data_sha256": sha256(source),
            "prediction_runs": {algorithm: {"path": str(run), "manifest_sha256": sha256(run / "run_manifest.json")} for algorithm, run in prediction_runs[dataset].items()},
        })
        write_json(dataset_dir / "artifact_hashes.json", {str(path): sha256(path) for path in dataset_dir.rglob("*") if path.is_file() and path.name != "artifact_hashes.json"})
    ...
    for metric in ("ari", "macro_precision", "macro_recall", "macro_f1", "hungarian_accuracy"):
        results_frame[f"delta_{metric}_from_baseline"] = [float(row[metric] - baseline.loc[(row.dataset, row.algorithm), metric]) for row in results_frame.itertuples()]
    results_frame.to_csv(output / "scenario_metrics.csv", index=False)
    pd.concat(population_results, ignore_index=True).to_csv(output / "scenario_population_metrics.csv", index=False)
    random_summary = results_frame[results_frame.kind == "random_control_flip"].groupby(["dataset", "algorithm"])[[f"delta_{metric}_from_baseline" for metric in ("ari", "macro_f1", "hungarian_accuracy")]].agg(["mean", "std", "min", "max"])
    ...
    manifest = {
        "experiment_id": "EXP-019", "completed_utc": datetime.now(timezone.utc).isoformat(),
        "config": str(config), "config_sha256": sha256(config), "protocol": str(protocol), "protocol_sha256": sha256(protocol),
        "script": str(Path(__file__).resolve()), "script_sha256": sha256(Path(__file__).resolve()),
        "prediction_runs": {dataset: {algorithm: str(path) for algorithm, path in runs.items()} for dataset, runs in prediction_runs.items()},
        "datasets": list(DATASETS), "algorithms": ["X-shift", "FlowSOM"], "knn_k": 15,
