...
def evaluate_rare(
    ...
    frame = (
        pd.DataFrame(rows)
        .sort_values(["f1", "recall", "predicted_cluster"], ascending=[False, False, True])
    ...
    metrics = {
        ...
        "n_predicted_metaclusters": int(np.unique(clusters).size),
        "target": target,
...
def main() -> int:
    ...
    parent, parent_verification, protocol, output = (
        args.parent.resolve(),
        args.parent_verification.resolve(),
        args.protocol.resolve(),
        args.output.resolve(),
    )
    progress = json.loads((parent / "progress_manifest.json").read_text(encoding="utf-8"))
    verification = json.loads(
        (parent_verification / "run_manifest.json").read_text(encoding="utf-8")
    )
    if (
        progress.get("all_runs_complete_and_passed") is not True
    ...
    for dataset, (data_path, target, target_events) in DATASETS.items():
        ...
        for seed in range(30):
            ...
            input_rows.append(
                {"dataset": dataset, "som_seed": seed, "codes_path": str(csv_path.resolve())}
            )
    ...
    with (
        (output / "r_stdout.log").open("w", encoding="utf-8") as stdout,
        (output / "r_stderr.log").open("w", encoding="utf-8") as stderr,
    ):
    ...
    for row in r_summary.itertuples(index=False):
        ...
        if node_map.shape != (100,):
            raise ValueError(f"mapping shape failed: {row.dataset}:{row.som_seed}:{row.regime}")
        ...
        metric_rows.append(
            {
                ...
                "label_information_used_for_regime_definition": uses_label_k,
    ...
    pd.concat(cluster_frames, ignore_index=True).to_csv(
        output / "metacluster_target_diagnostics.csv", index=False
    )
                ...
                pair_rows.append(
                    {
                        ...
                        "selector_seed_a": i,
                        "selector_seed_b": j,
                        "partition_ari": float(adjusted_rand_score(archive[i], archive[j])),
                    }
                )
    ...
    check(
        ...
        f"{int(r_checks.passed.sum())}/{len(r_checks)}",
    )
    check(
        "row_counts",
        len(metrics_frame) == 300 and len(main) == 240,
    ...
    check(
        "target_counts",
        set(metrics_frame[metrics_frame.dataset == "Nilsson_rare"].target_events) == {358}
        and set(metrics_frame[metrics_frame.dataset == "Mosmann_rare"].target_events) == {109},
        "358;109",
    )
    check(
        "fixed_k_contract",
        set(metrics_frame[metrics_frame.regime == "official_fixed_ktrue_2"].selected_k) == {2}
        and set(metrics_frame[metrics_frame.regime == "official_fixed_k10"].selected_k) == {10}
        and set(metrics_frame[metrics_frame.regime == "official_fixed_k40"].selected_k) == {40},
        "2/10/40",
    )
    check(
        "label_permission",
        metrics_frame.loc[
            metrics_frame.regime == "official_fixed_ktrue_2",
            "label_information_used_for_regime_definition",
        ].all()
        and not metrics_frame.loc[
            metrics_frame.regime != "official_fixed_ktrue_2",
            "label_information_used_for_regime_definition",
        ].any()
        and not metrics_frame.labels_used_by_r_metaclustering.any(),
        "only Ktrue regime definition uses label count",
    )
    check(
        "metric_ranges",
    ...
    check(
        "partition_archives",
        len(list(event_dir.glob("*.npz"))) == 10
        and all(
    ...
    check(
        "paired_design",
        len(paired_rows) == 900 and len(contrast_rows) == 30,
        f"{len(paired_rows)};{len(contrast_rows)}",
    )
    check(
        "selector_pairs",
    ...
    result = {
        ...
        "parent": str(parent),
        "parent_progress_sha256": sha256(parent / "progress_manifest.json"),
        "parent_verification": str(parent_verification),
        "parent_verification_manifest_sha256": sha256(parent_verification / "run_manifest.json"),
    ...
    (output / "scientific_summary.md").write_text(
        f"# {args.experiment_id} FlowSOM metaclustering regimes for rare-population datasets\n\nAutomatic-K counts: {auto_counts}.\n\nThe analysis contains 300 event partitions. Checks passed: {result['checks_passed']}/{result['checks_total']}. Complete target precision, recall, F1, F2, and paired differences are provided in machine-readable tables.\n",
        encoding="utf-8",
    )
