...
def main() -> int:
    ...
    for analysis in inputs:
        ...
        check(dataset, "parent_artifact_hashes", not hash_failures, json.dumps(hash_failures, ensure_ascii=False))

        cells = pd.read_csv(cells_path)
        cell_pairs = set(zip(cells["grid_side"].astype(int), cells["rlen"].astype(int)))
        check(
            dataset,
        ...
        actual_pairs = set(zip(selected["contrast"], selected["metric"]))
        check(
            dataset,
            "main_contrast_set_complete",
            len(selected) == 12 and actual_pairs == expected_pairs and selected["n_paired_seeds"].astype(int).eq(5).all(),
    ...
    check("experiment", "two_distinct_expected_datasets", set(datasets) == {"Levine_32dim", "Samusik_01"}, json.dumps(datasets))
    ...
    for contrast in CONTRASTS:
        for metric in METRICS:
            rows = contrasts_out[(contrasts_out["contrast"] == contrast) & (contrasts_out["metric"] == metric)]
            values = {
            ...
            direction_rows.append(
                {
                    ...
                    "raw_direction_concordant": raw_direction(a) == raw_direction(b) and raw_direction(a) != "zero",
                    "levine32_outcome_direction": outcome_direction(metric, a),
                    "samusik_outcome_direction": outcome_direction(metric, b),
                    "both_improved": outcome_direction(metric, a) == outcome_direction(metric, b) == "improved",
    ...
    check("experiment", "direction_table_complete", len(directions) == 12 and not directions.isna().any().any(), f"rows={len(directions)}")
    ...
    summary_lines = [
        "# EXP-014C FlowSOM 因子效应跨数据集描述性核对",
        "",
        f"两个父分析的核验与制品哈希均通过；本次新增核验 {int(verification_out['passed'].sum())}/{len(verification_out)} 通过。",
        f"四个预注册主对比 × 三个终点共12项中，两个数据集原始效应方向一致 {concordant}/12，同时按终点含义改善 {both_improved}/12。",
        "",
    ]
    for metric, row in per_metric.iterrows():
        summary_lines.append(
            f"- {metric}: 方向一致 {int(row['concordant'])}/4；两数据集均改善 {int(row['both_improved'])}/4。"
        )
    summary_lines.extend(
        [
            "",
            "本结果只说明在这两个固定数据集、固定20,000事件训练制度和40元簇设定下的复现方向。两个数据集不足以支持总体推断，未做跨数据集显著性检验，也不得据外部标签挑选所谓最佳参数。",
        ]
    )
    ...
    (output / "artifact_hashes.json").write_text(
        json.dumps({str(path): sha256(path) for path in artifact_paths}, ensure_ascii=False, indent=2) + "\n",
        encoding="utf-8",
    )
