...
def load_dataset(root: Path, name: str, spec: dict):
    ...
    if spec["label_policy"] == "integer_1_to_24_evaluable":
        ...
        y = np.array([str(int(value)) if keep else "__unassigned__" for value, keep in zip(numeric, mask)])
    else:
...
def align_and_score(y_true: np.ndarray, y_pred: np.ndarray):
    ...
    mapping = {pred_labels[col]: true_labels[row] for row, col in zip(rows, cols)}
    ...
    for label in true_labels:
        ...
        per_population.append({
            "population": label, "tp": tp, "fp": fp, "fn": fn,
...
def main() -> int:
    ...
    for dataset in args.datasets:
        ...
        input_rows.append({
            ...
            "sha256": sha256(path), "total_cells": len(y),
            "evaluated_cells": int(evaluable.sum()), "markers": len(markers),
        ...
        for seed in range(args.seeds):
            for policy in ("all_cells_fit", "labeled_only_fit"):
                ...
                model = KMeans(
                    n_clusters=TRUE_K[dataset], n_init=1, max_iter=args.max_iter,
                ...
                prediction_path = args.output / "predictions" / f"{dataset}_{policy}_seed{seed:02d}.npz"
                np.savez_compressed(
                    prediction_path,
    ...
    paired = runs.pivot_table(
        index=["dataset", "seed"], columns="fit_policy",
    ...
    summary = paired.groupby("dataset").agg(
        ...
        all_better_ari_fraction=("delta_ari_all_minus_labeled", lambda values: float(np.mean(values > 0))),
        all_better_f1_fraction=("delta_macro_f1_all_minus_labeled", lambda values: float(np.mean(values > 0))),
    ).reset_index()
    ...
    lines = [
        f"# {args.experiment_id} 未标注细胞纳入政策成对实验",
        "",
        "对应审稿问题：13；同时提供问题 21 的等种子运行证据。",
        "",
        "该实验使用 K=true，因此只属于输入政策敏感性分析，不能作为无标签主比较。",
        "",
        "## 结果",
        "",
    ]
    for row in summary.itertuples(index=False):
        lines.append(
            f"- {row.dataset}：{row.seeds} 个成对种子；全部细胞拟合相对仅标注细胞拟合，"
            f"ARI 平均差 {row.mean_delta_ari:+.6f}，Macro F1 平均差 {row.mean_delta_macro_f1:+.6f}。"
        )
    lines.extend([
        "",
        "## 验收边界",
        "",
        "- 所有运行使用完全相同的算法、K、参数、种子与评价细胞，仅改变拟合集合。",
        "- 每次运行保存全部事件的簇标签、聚类中心、inertia 和迭代数。",
        "- 本实验量化标签过滤造成的偏差，但不决定主比较中的最佳算法或论文叙事。",
    ])
    (args.output / "audit.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
    print(json.dumps({"output": str(args.output), "runs": len(runs), "exit_code": 0}, ensure_ascii=False))
    return 0
