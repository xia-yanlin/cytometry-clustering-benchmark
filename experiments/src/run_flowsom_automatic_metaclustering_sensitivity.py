...
PARENTS = {
    "Levine_32dim": ("EXP-021A_flowsom_levine32_stability30_20260910", 14, DATA_ROOT / "Levine_32dim" / "Levine_32dim_notransform.txt"),
    "Samusik_01": ("EXP-021B_flowsom_samusik_stability30_20260910", 24, DATA_ROOT / "Samusik_01" / "Samusik_01_notransform.txt"),
}
...
def main() -> int:
    ...
    workspace, protocol, output = args.workspace.resolve(), args.protocol.resolve(), args.output.resolve()
    ...
    for dataset, (parent_name, true_k, data_path) in PARENTS.items():
        ...
        for seed in range(30):
            ...
            input_rows.append({"dataset": dataset, "som_seed": seed, "codes_path": str(csv_path.resolve()), "true_k": true_k})
    ...
    if completed.returncode != 0:
        write_json(output / "failure.json", {"stage": "R automatic metaclustering", "exit_code": completed.returncode, "scientific_output_eligible": False})
        return 1
    ...
    for row in r_summary.itertuples(index=False):
        ...
        subset = mappings[
            (mappings.dataset == row.dataset) & (mappings.som_seed == row.som_seed) &
            (mappings.regime == row.regime) & (mappings.selector_seed == selector_seed)
        ...
        metrics, populations, clusters, assignment, _ = evaluate_multiclass(labels[evaluable], event_labels[evaluable], event_labels)
        ok, detail = metrics_are_valid(metrics)
        if not ok: raise ValueError(detail)
        metric_rows.append({
            "dataset": row.dataset, "som_seed": int(row.som_seed), "regime": row.regime,
            "selector_seed": selector_seed, "requested_k": int(row.requested_k), "selected_k": int(row.selected_k),
            ...
            "r_runtime_seconds": float(row.r_runtime_seconds), "label_information_used_for_selection": False, **metrics,
        })
        keys = {"dataset": row.dataset, "som_seed": int(row.som_seed), "regime": row.regime, "selector_seed": selector_seed}
    ...
    pd.concat(cluster_frames, ignore_index=True).to_csv(output / "metacluster_level_diagnostics.csv", index=False)
    pd.concat(assignment_frames, ignore_index=True).to_csv(output / "hungarian_mappings.csv", index=False)
    ...
    for (dataset, regime), group in metrics_frame.groupby(["dataset", "regime"], sort=True):
        for metric in ("selected_k", *METRICS):
            ...
            distribution_rows.append({"dataset": dataset, "regime": regime, "metric": metric, "n": len(values), "mean": float(values.mean()), "sd": float(values.std(ddof=1)), "median": float(np.median(values)), "min": float(values.min()), "max": float(values.max()), "q25": float(np.quantile(values, .25)), "q75": float(np.quantile(values, .75))})
    ...
    for dataset in PARENTS:
        ...
        for i in range(30):
            for j in range(i + 1, 30):
                pair_rows.append({"dataset": dataset, "selector_seed_a": i, "selector_seed_b": j, "partition_ari": adjusted_rand_score(archive[i], archive[j])})
    ...
    check("r_checks", len(r_checks) == 9 and bool(r_checks.passed.all()), f"passed={int(r_checks.passed.sum())}/{len(r_checks)}")
    check("row_counts", len(metrics_frame) == 240 and len(main) == 180 and int((metrics_frame.regime == SENSITIVITY_REGIME).sum()) == 60, f"total={len(metrics_frame)}; main={len(main)}")
    ...
    check("selection_range", metrics_frame.selected_k.between(2, 40).all(), f"range={metrics_frame.selected_k.min()}-{metrics_frame.selected_k.max()}")
    check("label_information", not metrics_frame.label_information_used_for_selection.any(), "all false")
    check("metric_ranges", metrics_frame[list(METRICS)].apply(lambda col: col.between(-1 if col.name == "ari" else 0, 1).all()).all(), "all endpoints in legal ranges")
    check("partition_archives", len(list(event_dir.glob("*.npz"))) == 8 and all(np.load(path, allow_pickle=False)["partitions"].shape[0] == 30 for path in event_dir.glob("*.npz")), "8 archives x 30 partitions")
    check("paired_design", len(paired_rows) == 600 and len(contrast_rows) == 20, f"paired={len(paired_rows)}; summaries={len(contrast_rows)}")
    check("selector_pair_counts", len(pair_frame) == 870 and pair_frame.partition_ari.between(-1, 1).all(), f"pairs={len(pair_frame)}")
    check("parent_hash_rows", len(parent_hashes) == 180 and all(Path(row["path"]).is_file() and sha256(Path(row["path"])) == row["sha256"] for row in parent_hashes), f"hashes={len(parent_hashes)}")
    checks_frame = pd.DataFrame(checks)
    checks_frame.to_csv(output / "checks.csv", index=False)
    all_passed = bool(checks_frame.passed.all())
    result = {
        "experiment_id": "EXP-032", "completed_utc": datetime.now(timezone.utc).isoformat(),
    ...
    selector_counts = metrics_frame[metrics_frame.regime == SENSITIVITY_REGIME].groupby(["dataset", "selected_k"]).size().to_dict()
    (output / "scientific_summary.md").write_text(
        "# EXP-032 FlowSOM官方自动元簇数敏感性\n\n"
        f"主配对自动k计数：{auto_counts}。\n\n选择器seed敏感性k计数：{selector_counts}。\n\n"
        f"共{result['checks_passed']}/{result['checks_total']}项检查通过；自动、K=true与固定40的完整指标及配对差保存在机器可读表中。\n",
        encoding="utf-8",
    )
