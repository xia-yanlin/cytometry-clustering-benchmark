...
def main() -> int:
    ...
    if (
        parent_matrix.shape != (86864, 39)
        or len(parent_names) != 39
        or parent_labels.shape != (86864,)
    ):
    ...
    if (
        ...
        or not random_contract["source_has_math_random_fallback"]
    ...
    if (
        len(truth) != 86864
        or int(evaluable.sum()) != 53173
        or np.unique(truth[evaluable]).size != 24
    ):
    ...
    for row in results:
        ...
        metrics, populations, _, _, _ = evaluate_multiclass(
        ...
        populations.insert(0, "repeat", repeat)
    ...
    unique_hashes = int(run_frame.partition_sha256.nunique()) if len(run_frame) else 0
    checks = []
    ...
    check(
        "parent_manifest",
        parent_manifest["experiment_id"] == "EXP-010A" and parent_manifest["all_checks_passed"],
    ...
    check(
        "population_metrics",
        sum(len(frame) for frame in population_frames) == successful * 24,
        f"rows={sum(len(frame) for frame in population_frames)}",
    )
    check(
        "pairwise_count",
    ...
    check(
        "labels_valid",
        len(run_frame) == 30 and run_frame.n_predicted_clusters_all_events.ge(1).all(),
        f"cluster_range={run_frame.n_predicted_clusters_all_events.min() if len(run_frame) else None}-{run_frame.n_predicted_clusters_all_events.max() if len(run_frame) else None}",
    )
    check(
    ...
    (output / "scientific_summary.md").write_text(
        "# EXP-039 Thirty native X-shift repeats on Samusik at fixed K=20\n\n"
        f"Successful runs: {successful}/30; unique all-event partition hashes: {unique_hashes}; mean/range of the 435 pairwise partition ARI values={pair_values.mean():.6f}/[{pair_values.min():.6f}, {pair_values.max():.6f}]. "
        f"Mean/range of macro F1={run_frame.macro_f1.mean():.6f}/[{run_frame.macro_f1.min():.6f}, {run_frame.macro_f1.max():.6f}]. "
        "The CLI exposes no seed and the source/JAR uses a no-argument Random instance, so run numbers denote native repeats only.\n",
        encoding="utf-8",
    )
    artifacts = [p for p in output.rglob("*") if p.is_file()]
    (output / "artifact_hashes.json").write_text(
        json.dumps(
            {str(p.relative_to(output)): sha256(p) for p in artifacts}, ensure_ascii=False, indent=2
        )
        + "\n",
        encoding="utf-8",
    )
