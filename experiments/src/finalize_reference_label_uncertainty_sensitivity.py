...
def main() -> int:
    ...
    parent_ok = failure.get("status") == "event_level_scenarios_complete_top_level_summary_failed" and failure.get("dataset_scenario_artifacts_complete") is True
    ...
    for dataset in DATASETS:
        ...
        source_ok = str(source.resolve()) == str(Path(source_links["data"]).resolve()) and sha256(source) == source_links["data_sha256"]
        labels_ok = np.array_equal(labels, expected_labels) and np.array_equal(evaluation_indices, np.flatnonzero(evaluable))
        check(dataset, "source_and_reference", source_ok and labels_ok, f"source={sha256(source)}; events={len(labels)}; markers={len(markers)}")
        ...
        for algorithm, link in source_links["prediction_runs"].items():
            ...
            check(dataset, f"{algorithm}_prediction_link", manifest_ok and len(full_predictions) == len(matrix), f"run={run}; events={len(full_predictions)}")
        ...
        uncertainty_ok = neighbors.shape == (len(labels), 15) and not np.any(neighbors == np.arange(len(labels))[:, None])
        uncertainty_ok = uncertainty_ok and disagreement.shape == (len(labels),) and disagreement.min() >= 0 and disagreement.max() <= 1
        uncertainty_ok = uncertainty_ok and alternative.shape == has_alternative.shape == (len(labels),)
        check(dataset, "uncertainty_arrays", uncertainty_ok, f"neighbors={neighbors.shape}; disagreement={float(disagreement.min())}-{float(disagreement.max())}; alternatives={int(has_alternative.sum())}")

        scenario_table = pd.read_csv(dataset_dir / "scenario_manifest.csv")
        scenario_set_ok = len(scenario_table) == 37 and (scenario_table["kind"] == "random_control_flip").sum() == 30
        check(dataset, "scenario_set", scenario_set_ok, str(scenario_table.groupby("kind").size().to_dict()))
        for scenario in scenario_table.itertuples(index=False):
            ...
            scenario_ok = sha256(labels_path) == scenario.labels_sha256 and sha256(mask_path) == scenario.mask_sha256
            scenario_ok = scenario_ok and scenario_labels.shape == keep.shape == labels.shape and changed == int(scenario.changed_labels) and excluded == int(scenario.excluded_events)
            check(dataset, f"scenario_{scenario.scenario}", scenario_ok, f"changed={changed}; excluded={excluded}; retained={int(keep.sum())}")
            for algorithm, predicted in predictions.items():
                ...
                results.append({"dataset": dataset, "algorithm": algorithm, "scenario": scenario.scenario, "kind": scenario.kind, "nominal_fraction": float(scenario.nominal_fraction), "seed": scenario.seed, "changed_labels": changed, "excluded_events": excluded, **metrics})
    ...
    baseline = results_frame.loc[results_frame["kind"] == "unchanged", ["dataset", "algorithm", *METRICS]].rename(columns=baseline_columns)
    results_frame = results_frame.merge(baseline, on=["dataset", "algorithm"], how="left", validate="many_to_one")
    for metric in METRICS:
        results_frame[f"delta_{metric}_from_baseline"] = results_frame[metric] - results_frame[f"baseline_{metric}"]
    results_frame.to_csv(output / "scenario_metrics.csv", index=False)
    pd.concat(populations, ignore_index=True).to_csv(output / "scenario_population_metrics.csv", index=False)

    delta_columns = ["delta_ari_from_baseline", "delta_macro_f1_from_baseline", "delta_hungarian_accuracy_from_baseline"]
    random_summary = results_frame.loc[results_frame["kind"] == "random_control_flip"].groupby(["dataset", "algorithm"])[delta_columns].agg(["mean", "std", "min", "max"])
    random_summary.columns = [f"{metric}_{statistic}" for metric, statistic in random_summary.columns]
    random_summary.reset_index().to_csv(output / "random_control_summary.csv", index=False)
    results_frame.loc[results_frame["kind"].isin(["unchanged", "boundary_flip", "boundary_exclude"])].to_csv(output / "deterministic_scenario_metrics.csv", index=False)
    ...
    ten_percent = results_frame.loc[results_frame["scenario"] == "boundary_flip_10pct", ["dataset", "algorithm", "delta_ari_from_baseline", "delta_macro_f1_from_baseline"]]
    lines = ["# EXP-019-R2 参考标签不确定性敏感性", "", f"父场景未重建；从事件级标签/掩码复算两个数据集、两算法、各37场景。收尾检查{manifest['checks_passed']}/{manifest['checks_total']}。", "", "10%边界翻转相对基线："]
    for row in ten_percent.itertuples(index=False):
        lines.append(f"- {row.dataset} / {row.algorithm}: ΔARI={row.delta_ari_from_baseline:+.6f}，ΔMacro F1={row.delta_macro_f1_from_baseline:+.6f}。")
    lines.extend(["", "这些扰动比例不是实际误标率；结果不能替代真实多门控者一致性数据。"])
