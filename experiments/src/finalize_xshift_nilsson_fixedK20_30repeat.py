...
def main() -> int:
    ...
    random_contract = json.loads(
        (failed_parent / "randomness_source_contract.json").read_text(encoding="utf-8")
    )
    ...
    for repeat in range(30):
        ...
        if (
            ...
            or not manifest["all_checks_passed"]
        ...
        if (
            ...
            or names[-1].lower().replace("_", "") != "clusterid"
            or not np.array_equal(np.rint(matrix[:, -1]).astype(np.int32), labels)
        ):
        ...
        for role, path in (
            ...
            ("labels", labels_path),
            ("output_fcs", output_fcs[0]),
            ("stderr", run_dir / "xshift_stderr.log"),
        ):
            parent_rows.append(
                {
                    "repeat": repeat,
    ...
    check(
        "failed_parent_has_no_top_manifest",
        not (failed_parent / "run_manifest.json").exists(),
        "aggregation failed after 30 subruns",
    )
    check(
        "parent_repeat_count",
        len(run_frame) == 30 and set(run_frame.repeat) == set(range(30)),
        f"rows={len(run_frame)}",
    )
    check("parent_artifact_count", len(parent_rows) == 120, f"rows={len(parent_rows)}")
    check(
        "randomness_contract",
        random_contract["tag_commit"] == "fda75cf79980222da663185e2a6a72b442b9aff3"
        and random_contract["source_has_shuffle_initialization"]
        and random_contract["default_random_constructor_present"]
        and not random_contract["seeded_random_constructor_present"]
        and not random_contract["cli_seed_interface"],
    ...
    check(
        ...
        np.isfinite(
            run_frame[
                ["ari", "target_precision", "target_recall", "target_f1", "target_f2"]
            ].to_numpy()
        ).all(),
        "all finite",
    )
    ...
    manifest = {
        "experiment_id": "EXP-038-R1",
        "completed_utc": datetime.now(timezone.utc).isoformat(),
        "protocol": str(protocol),
        "protocol_sha256": sha256(protocol),
        "script": str(Path(__file__).resolve()),
        "script_sha256": sha256(Path(__file__).resolve()),
        "corrected_runner": str(source_runner),
        "corrected_runner_sha256": sha256(source_runner),
        "failed_parent": str(failed_parent),
        "randomness_source_contract_sha256": sha256(
            failed_parent / "randomness_source_contract.json"
        ),
        "input_hashes_sha256": sha256(failed_parent / "input_hashes.json"),
        "data": str(data_path),
    ...
    (output / "scientific_summary.md").write_text(
        "# EXP-038-R1 Thirty native X-shift repeats on Nilsson at fixed K=20\n\n"
        f"All 30 independent Java processes succeeded; there were {unique_hashes} unique all-event partition hashes. The mean/range of the 435 pairwise partition ARI values was {pair_values.mean():.6f}/[{pair_values.min():.6f}, {pair_values.max():.6f}]. "
        f"The mean/range of target F1 was {f1.mean():.6f}/[{f1.min():.6f}, {f1.max():.6f}]. "
        "The official CLI exposes no seed and the source uses default Random initialization, so these can only be described as native repeats.\n",
        encoding="utf-8",
    )
