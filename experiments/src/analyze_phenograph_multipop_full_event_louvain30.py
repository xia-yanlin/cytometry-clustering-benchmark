...
ENDPOINTS = [
    "ari", "macro_precision", "macro_recall", "macro_f1", "weighted_f1",
    "hungarian_accuracy", "quality_q", "n_valid_communities",
    "overclustering_ratio_evaluable", "mean_population_effective_predicted_clusters",
    "weighted_evaluable_cluster_purity", "mean_cluster_effective_true_populations",
]
...
def main() -> int:
    ...
    for dataset, parent in parents.items():
        ...
        check(dataset, "parent_complete", progress.get("all_runs_complete_and_passed") is True and len(index) == 30 and passed.all(), f"rows={len(index)}; passed={int(passed.sum())}")
        check(dataset, "repeat_contract", index["repeat"].astype(int).tolist() == list(range(30)), str(index["repeat"].astype(int).tolist()))
        check(dataset, "method_contract", index["algorithm"].eq("PhenoGraph_v1.5.7_default_Louvain").all() and index["k_neighbors"].astype(int).eq(30).all() and index["evaluation_scope"].eq("all_events_fit_evaluable_reference_labels_only").all(), "official_default_louvain_k30_full_event")
        ...
        for repeat in range(30):
            ...
            labels_by_repeat[repeat] = np.load(run / "community_labels_all_events.npy", allow_pickle=False)
        ...
        for first in range(30):
            for second in range(first + 1, 30):
                pair_rows.append({
                    ...
                    "repeat_a": first,
                    "repeat_b": second,
                    "partition_ari_all_events": float(adjusted_rand_score(labels_by_repeat[first], labels_by_repeat[second])),
                })
    ...
    pairs = pd.DataFrame(pair_rows).sort_values(["dataset", "repeat_a", "repeat_b"]).reset_index(drop=True)
    points.to_csv(output / "all_run_points.csv", index=False)
    pairs.to_csv(output / "pairwise_partition_ari.csv", index=False)
    check("experiment", "run_and_pair_counts", len(points) == 90 and len(pairs) == 1305 and pairs.groupby("dataset").size().eq(435).all(), f"runs={len(points)}; pairs={len(pairs)}")
    ...
    for dataset, group in points.groupby("dataset", sort=True):
        ...
        for endpoint in ENDPOINTS:
            ...
            resamples = values[rng.integers(0, len(values), size=(BOOTSTRAP_REPLICATES, len(values)))]
            bootstrap_means = resamples.mean(axis=1)
            q1, q3 = np.quantile(values, [0.25, 0.75])
            descriptive_rows.append({
                "dataset": dataset, "endpoint": endpoint, "n_runs": len(values),
                "mean": float(values.mean()), "sd": float(values.std(ddof=1)),
                "median": float(np.median(values)), "mad_unscaled": float(np.median(np.abs(values - np.median(values)))),
        ...
        for endpoint in CORRELATION_ENDPOINTS:
            ...
            correlation_rows.append({"dataset": dataset, "x": "quality_q", "y": endpoint, "n_runs": len(group), "correlation_defined": defined, "undefined_reason": reason, "spearman_rho": rho, "p_raw": p_raw})
    ...
    correlations["p_holm_across_nine"] = holm_adjust(correlations["p_for_holm"].to_numpy(dtype=float))
    correlations["reject_0_05_holm"] = correlations["correlation_defined"] & correlations["p_holm_across_nine"].le(0.05)
    ...
    pair_summary = pairs.groupby("dataset")["partition_ari_all_events"].agg(["mean", "std", "median", "min", "max"]).reset_index()
    pair_summary["pairs"] = 435
    pair_summary.to_csv(output / "pairwise_partition_ari_summary.csv", index=False)
    check("experiment", "descriptive_family", len(descriptive) == 36 and descriptive.groupby("dataset").size().eq(12).all(), f"rows={len(descriptive)}")
    check("experiment", "correlation_family", len(correlations) == 9 and correlations.groupby("dataset").size().eq(3).all(), f"rows={len(correlations)}")
    check("experiment", "metric_ranges", points["ari"].between(-1, 1).all() and points["macro_f1"].between(0, 1).all() and points["hungarian_accuracy"].between(0, 1).all() and pairs["partition_ari_all_events"].between(-1, 1).all(), "all legal")
    ...
    manifest = {
        ...
        "source_links": source_links, "endpoints": ENDPOINTS, "correlation_endpoints": CORRELATION_ENDPOINTS,
        "repeat_not_seed": True, "evaluation_scope": "all_events_fit_evaluable_reference_labels_only",
        "bootstrap_seed": BOOTSTRAP_SEED, "bootstrap_replicates": BOOTSTRAP_REPLICATES,
        "checks_passed": int(check_frame["passed"].sum()), "checks_total": len(check_frame),
    ...
    summary_lines = ["# EXP-045D PhenoGraph 多群体全事件汇总", "", "90 个父运行、1,305 个全事件分区对、36 个描述性终点和 9 项 Q—外部指标关系。", ""]
    for row in pair_summary.itertuples(index=False):
        summary_lines.append(f"- {row.dataset}: pairwise ARI mean={row.mean:.6f}, range=[{row.min:.6f}, {row.max:.6f}].")
    summary_lines.extend(["", f"检查 {manifest['checks_passed']}/{manifest['checks_total']}；repeat 不是受控 seed，不筛选最佳 repeat。", ""])
    (output / "scientific_summary.md").write_text("\n".join(summary_lines), encoding="utf-8")
    write_json(output / "artifact_hashes.json", {path.name: sha256(path) for path in output.iterdir() if path.is_file() and path.name != "artifact_hashes.json"})
