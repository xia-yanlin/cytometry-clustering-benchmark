...
ENDPOINTS = ["ari", "macro_f1", "weighted_f1", "hungarian_accuracy", "median_bmu_distance", "runtime_seconds"]
N_BOOT = 10_000
RNG_SEED = 20_260_916
...
def describe(values: np.ndarray, rng: np.random.Generator) -> dict[str, float]:
    ...
    return {
        "n": len(values), "mean": values.mean(), "sd": values.std(ddof=1), "median": np.median(values),
        "q1": q1, "q3": q3, "min": values.min(), "max": values.max(),
        "bootstrap_mean_ci95_low": np.quantile(boot, .025),
        "bootstrap_mean_ci95_high": np.quantile(boot, .975),
    }
...
def main() -> int:
    ...
    for (dataset, scheme), (run_name, kind) in SOURCES.items():
        ...
        design_ok = design_ok and index["all_checks_passed"].astype(str).str.lower().eq("true").all()
        design_ok = design_ok and progress.get("all_runs_complete_and_passed") is True
        check(f"{dataset}:{scheme}", "batch_contract", design_ok, f"rows={len(index)}; progress={progress.get('all_runs_complete_and_passed')}")
        top_inventory = run / "artifact_hashes.json"
        top_failures = inventory_failures(top_inventory) if top_inventory.is_file() else []
        check(
            f"{dataset}:{scheme}", "top_level_hashes_or_legacy_format", not top_failures,
        ...
        for seed in range(30):
            ...
            failures.extend(f"seed{seed:03d}:{item}" for item in inventory_failures(seed_dir / "artifact_hashes.json"))
        ...
        check(f"{dataset}:{scheme}", "run_hashes_manifests_and_labels", not failures, f"failures={len(failures)}")
        normalized = index[["seed", *ENDPOINTS, "n_total_events", "n_evaluable_events", "n_true_populations", "n_predicted_clusters_all_events"]].copy()
        ...
        source_links.append({
            "dataset": dataset, "scheme": scheme, "run": str(run),
            "progress_manifest_sha256": sha256(run / "progress_manifest.json"),
            "run_index_sha256": sha256(run / "run_index.csv"),
            "artifact_hashes_sha256": sha256(top_inventory) if top_inventory.is_file() else None,
            "run_manifest_sha256_by_seed": run_hashes,
        })

    points = pd.concat(point_frames, ignore_index=True).sort_values(["dataset", "scheme", "seed"]).reset_index(drop=True)
        ...
        for scheme in ["Python_20k_train", "R_full_event"]:
            for left in range(30):
                for right in range(left + 1, 30):
                    within_rows.append({
                        "dataset": dataset, "scheme": scheme, "seed_a": left, "seed_b": right,
                        "partition_ari_all_events": adjusted_rand_score(labels[(dataset, scheme, left)], labels[(dataset, scheme, right)]),
                    })
    ...
    for dataset in sorted({key[0] for key in SOURCES}):
        for seed in range(30):
            cross_rows.append({
                ...
                "partition_ari_r_full_vs_python_20k": adjusted_rand_score(labels[(dataset, "R_full_event", seed)], labels[(dataset, "Python_20k_train", seed)]),
            })
    ...
    for (dataset, scheme), group in points.groupby(["dataset", "scheme"], sort=True):
        for endpoint in ENDPOINTS:
            descriptive_rows.append({"dataset": dataset, "scheme": scheme, "endpoint": endpoint, **describe(group.sort_values("seed")[endpoint].to_numpy(float), rng)})
    ...
    for dataset in sorted(points["dataset"].unique()):
        ...
        for endpoint in ENDPOINTS:
            delta_rows.append({"dataset": dataset, "contrast": "R_full_event_minus_Python_20k_train", "endpoint": endpoint, **describe(r[endpoint].to_numpy(float) - py[endpoint].to_numpy(float), rng)})
    deltas = pd.DataFrame(delta_rows)
    deltas.to_csv(output / "paired_seed_descriptive_differences.csv", index=False)

    within_summary = within.groupby(["dataset", "scheme"])["partition_ari_all_events"].agg(["count", "mean", "std", "median", "min", "max"]).reset_index()
    within_summary.to_csv(output / "within_scheme_partition_ari_summary.csv", index=False)
    cross_summary = cross.groupby("dataset")["partition_ari_r_full_vs_python_20k"].agg(["count", "mean", "std", "median", "min", "max"]).reset_index()
    ...
    manifest = {
        "experiment_id": "EXP-046C", "completed_utc": datetime.now(timezone.utc).isoformat(),
        ...
        "source_links": source_links, "endpoints": ENDPOINTS, "bootstrap_replicates": N_BOOT,
        "rng_seed": RNG_SEED, "checks_passed": int(check_frame["passed"].sum()),
        "checks_total": len(check_frame), "all_checks_passed": all_passed,
    ...
    key = deltas[deltas["endpoint"].isin(["ari", "macro_f1", "hungarian_accuracy"])][["dataset", "endpoint", "mean", "bootstrap_mean_ci95_low", "bootstrap_mean_ci95_high"]]
    stability = within_summary[["dataset", "scheme", "mean", "min", "max"]]
    agreement = cross_summary[["dataset", "mean", "min", "max"]]
    summary = [
        "# EXP-046C 纯R全事件FlowSOM外推性分析", "",
        "四个源批次共120个运行均纳入。实现、版本与训练范围同时改变，以下差异仅为外推性描述，不支持单因素归因或跨算法排名。", "",
        "## R全事件减Python 2万训练的同seed描述性差值", "", key.to_csv(index=False).strip(), "",
        "## 方法内30种子事件分区ARI", "", stability.to_csv(index=False).strip(), "",
        "## 同seed跨方案事件分区ARI", "", agreement.to_csv(index=False).strip(), "",
        "运行时间仅为本机执行记录，不作为通用速度基准。",
    ]
    (output / "scientific_summary.md").write_text("\n".join(summary) + "\n", encoding="utf-8")
    inventory = {str(path): sha256(path) for path in output.iterdir() if path.is_file()}
    (output / "artifact_hashes.json").write_text(json.dumps(inventory, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
