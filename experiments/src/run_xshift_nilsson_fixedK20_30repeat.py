...
def read_fcs(path: Path) -> tuple[np.ndarray, list[str]]:
    ...
    names = [data.channels[i].get("pnn") or data.channels[i].get("pns") or f"channel_{i}" for i in range(1, data.channel_count + 1)]
    return matrix, names
...
def class_methodrefs(data: bytes) -> list[tuple[str, str, str]]:
    ...
    while i < count:
        ...
        if tag == 1:
            ...
            cp[i] = (tag, data[pos:pos + 4]); pos += 4
        elif tag in (5, 6):
            cp[i] = (tag, data[pos:pos + 8]); pos += 8; i += 1
        elif tag in (7, 8, 16, 19, 20):
            cp[i] = (tag, struct.unpack_from(">H", data, pos)[0]); pos += 2
        elif tag in (9, 10, 11, 12, 17, 18):
            cp[i] = (tag, *struct.unpack_from(">HH", data, pos)); pos += 4
        elif tag == 15:
            cp[i] = (tag, data[pos], struct.unpack_from(">H", data, pos + 1)[0]); pos += 3
        else:
    ...
    for item in cp:
        if isinstance(item, tuple) and item[0] in (10, 11):
            ...
            if not isinstance(class_item, tuple) or class_item[0] != 7 or not isinstance(nt_item, tuple) or nt_item[0] != 12:
                raise ValueError("bad method reference")
            refs.append((utf(int(class_item[1])), utf(int(nt_item[1])), utf(int(nt_item[2]))))
    return refs


def target_metrics(truth: np.ndarray, clusters: np.ndarray, target: str = "HSCs") -> dict[str, float | int]:
    ...
    for cluster in np.unique(clusters):
        ...
        rows.append((f1, recall, -int(cluster), int(cluster), tp, fp, fn, precision, recall, f1, f2, int(predicted.sum())))
    best = max(rows)
    return {"selected_cluster": best[3], "target_tp": best[4], "target_fp": best[5], "target_fn": best[6], "target_precision": best[7], "target_recall": best[8], "target_f1": best[9], "target_f2": best[10], "selected_cluster_size": best[11]}
...
def main() -> int:
    ...
    if sha256(release_jar) != sha256(Path(json.loads((parent / "xshift_execution.json").read_text(encoding="utf-8"))["command"][3])):
    ...
    if tag_commit != "fda75cf79980222da663185e2a6a72b442b9aff3" or not random_contract["source_has_shuffle_initialization"] or not random_contract["default_random_constructor_present"] or random_contract["seeded_random_constructor_present"]:
    ...
    input_hashes = {"input_fcs": sha256(input_fcs), "config": sha256(config), "release_jar": sha256(release_jar), "java": sha256(java), "data": sha256(data_path)}
    ...
    def run_one(repeat: int) -> dict[str, object]:
        ...
        if not timed_out and exit_code == 0 and len(candidates) == 1:
            ...
            values = matrix[:, -1]
            success = matrix.shape == (44140, 14) and names[:-1] == parent_names and names[-1].lower().replace("_", "") == "clusterid" and np.all(np.isfinite(values)) and np.allclose(values, np.rint(values)) and np.array_equal(matrix[:, :-1], parent_matrix)
            if success:
    ...
    for row in results:
        ...
        run_rows.append({"repeat": repeat, "runtime_seconds": row["runtime_seconds"], "n_clusters": int(np.unique(labels).size), "partition_sha256": hashlib.sha256(labels.tobytes()).hexdigest(), "ari": adjusted_rand_score(truth, labels), **metrics})
    ...
    check("parent_manifest", parent_manifest["all_checks_passed"] and parent_manifest["experiment_id"] == "EXP-010B", parent_manifest["experiment_id"])
    check("randomness_source", all([random_contract["source_has_shuffle_initialization"], random_contract["source_has_math_random_fallback"], random_contract["default_random_constructor_present"], not random_contract["seeded_random_constructor_present"], not random_contract["cli_seed_interface"]]), json.dumps(random_contract))
    ...
    check("labels_valid", len(run_frame) == 30 and run_frame.n_clusters.ge(1).all(), f"cluster_range={run_frame.n_clusters.min() if len(run_frame) else None}-{run_frame.n_clusters.max() if len(run_frame) else None}")
    ...
    manifest = {
        "experiment_id": "EXP-038", "completed_utc": datetime.now(timezone.utc).isoformat(),
        ...
        "release_jar": str(release_jar), "release_jar_sha256": sha256(release_jar), "java": str(java), "java_sha256": sha256(java),
        "data": str(data_path), "data_sha256": sha256(data_path), "workers": args.workers, "repeat_not_seed": True,
        "runs_attempted": 30, "runs_successful": successful, "runs_failed": 30 - successful, "unique_partition_hashes": int(unique_hashes),
    ...
    (output / "scientific_summary.md").write_text(
        "# EXP-038 X-shift Nilsson固定K=20三十次原生重复\n\n"
        f"成功{successful}/30，唯一全事件分区哈希{unique_hashes}个。两两分区ARI均值/范围为{pair_values.mean():.6f}/[{pair_values.min():.6f}, {pair_values.max():.6f}]。"
        f"目标F1均值/范围为{f1.mean():.6f}/[{f1.min():.6f}, {f1.max():.6f}]。"
        "官方CLI无seed且源码存在未设种子的随机初始化，因此只能解释为独立进程原生repeat。\n",
        encoding="utf-8",
    )
