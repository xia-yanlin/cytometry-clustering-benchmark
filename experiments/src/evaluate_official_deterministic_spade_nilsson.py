...
PROTOCOL = (
    ROOT
    / "protocols"
    / "EXP-044-R4_deterministic_spade_nilsson_external_evaluation_preregistered.md"
)
...
def evaluate(
    truth: np.ndarray, predicted: np.ndarray, attempt: int
    ...
    best = max(
        rows, key=lambda row: (float(row["f1"]), float(row["recall"]), -int(row["cluster_id"]))
    )
    summary = {
        "attempt": attempt,
...
def main() -> int:
    ...
    check(
        "label_contract",
        len(truth) == EXPECTED_EVENTS and int(np.count_nonzero(truth == "HSCs")) == EXPECTED_TARGET,
        f"events={len(truth)} target={np.count_nonzero(truth == 'HSCs')}",
    )
    ...
    for attempt in (3, 4):
        ...
        check(
            f"attempt{attempt}_validated",
            bool(validation["all_checks_passed"]),
            str(validation["checks"]),
        )
        check(
            f"attempt{attempt}_event_contract",
            predicted.shape == (EXPECTED_EVENTS,) and np.unique(predicted).size == 40,
            f"shape={predicted.shape} clusters={np.unique(predicted).size}",
        )
    ...
    check(
        "repeat_external_metrics_exact",
        metrics_exact,
        str({key: summaries[0][key] for key in metric_fields}),
    )
    ...
    write_json(
        RUN / "external_evaluation_artifact_hashes.json",
        {
            name: sha256(RUN / name)
            for name in [
                "external_evaluation.json",
                "external_evaluation_per_cluster.csv",
                "external_evaluation_checks.csv",
            ]
        },
    )
