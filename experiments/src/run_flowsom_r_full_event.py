            ...
            rows.append(
                {
                    "run_name": run.name,
                    "seed": seed,
                    "runtime_seconds": manifest["r_runtime_seconds"],
...
def progress(
    ...
    value = {
        ...
        "fit_policy": "all_events_direct_training",
        "xdim": 10,
        "ydim": 10,
        "rlen": 30,
        "requested_metaclusters": 40,
        "seeds": list(SEEDS),
        "expected_runs": 30,
...
def main() -> int:
    ...
    rscript_exe = args.rscript_exe.resolve()
    ...
    if source_alias.stat().st_size != source.stat().st_size or sha256(source_alias) != sha256(
        source
    ):
    ...
    command = [
        ...
        str(r_script),
        str(source_alias),
        str(output / "marker_columns.txt"),
        str(cofactor),
        args.dataset,
        str(output),
        str(args.max_new_runs),
        sha256(protocol),
    ]
    with (
        (output / "r_batch_stdout.log").open("a", encoding="utf-8") as stdout,
        (output / "r_batch_stderr.log").open("a", encoding="utf-8") as stderr,
    ):
    ...
    if (
        len(labels) != contract["rows"]
        or int(evaluable.sum()) != contract["evaluable"]
        or len(np.unique(labels[evaluable])) != contract["populations"]
    ):
    ...
    for seed in SEEDS:
        ...
        metrics, populations, clusters, mapping, contingency = evaluate_multiclass(
            labels[evaluable], event_clusters[evaluable], event_clusters
        )
        ...
        check(
            "r_manifest",
            r_manifest.get("all_checks_passed") is True
            and r_manifest.get("package_version") == "2.18.0",
            str(r_manifest.get("package_version")),
        )
        check(
            "r_checks",
            r_checks["passed"].astype(str).str.lower().eq("true").all() and len(r_checks) == 10,
            f"passed={int(r_checks['passed'].astype(str).str.lower().eq('true').sum())}/10",
        )
        check(
            "event_shapes",
            node_labels.shape == event_clusters.shape == (contract["rows"],),
            str(event_clusters.shape),
        )
        check(
            "node_range",
            node_labels.min() >= 1 and node_labels.max() <= 100,
            str((node_labels.min(), node_labels.max())),
        )
        check(
            "metacluster_range",
            event_clusters.min() >= 1 and event_clusters.max() <= 40,
            str((event_clusters.min(), event_clusters.max())),
        )
        check(
            "code_shape",
            codes.shape == (100, contract["markers"]) and np.isfinite(codes).all(),
            str(codes.shape),
        )
        check(
            "node_mapping",
            ...
            and np.array_equal(
                event_clusters,
                node_meta["metacluster_label_1based"].to_numpy(dtype=np.int32)[node_labels - 1],
            ),
            "event labels reconstructed from R node outputs",
        )
        check(
            "evaluation_contract",
            ...
            f"events={metrics['n_total_events']}; evaluable={metrics['n_evaluable_events']}",
        )
        check("metric_ranges", metric_ok, metric_detail)
        check(
            "labels_permission",
            r_manifest.get("labels_used_for_fit") is False,
            "labels loaded only after R batch",
        )
        ...
        pd.DataFrame(
            [
                {
                    **r_summary,
                    "algorithm": "R_FlowSOM_2.18.0_full_event_fixedK40",
        ...
        clusters.to_csv(run / "metacluster_level_diagnostics.csv", index=False)
        ...
        manifest = {
            ...
            "source": str(source),
            "source_sha256": sha256(source),
            "r_input_ascii_hardlink": str(source_alias),
            "r_input_ascii_hardlink_sha256": sha256(source_alias),
            "protocol": str(protocol),
            "protocol_sha256": sha256(protocol),
            "orchestrator": str(Path(__file__).resolve()),
            "orchestrator_sha256": sha256(Path(__file__).resolve()),
            ...
            json.dumps(
                {
                    "dataset": args.dataset,
                    "seed": seed,
                    "r_runtime_seconds": manifest["r_runtime_seconds"],
                    "ari": metrics["ari"],
