...
def main() -> int:
    ...
    for name in ("parent", "source-repo", "release-jar", "java", "data", "protocol", "output"):
    ...
    ap.add_argument("--label-mode", choices=["numeric_1_to_n", "not_unassigned"], required=True)
    ap.add_argument("--n-events", type=int, required=True)
    ap.add_argument("--n-markers", type=int, required=True)
    ap.add_argument("--n-populations", type=int, required=True)
    ap.add_argument("--expected-evaluable", type=int, required=True)
    ...
    matrix, names = read_fcs(input_fcs)
    if (
        pm["experiment_id"] != a.parent_id
        or not pm["all_checks_passed"]
        or matrix.shape != (a.n_events, a.n_markers)
    ):
        raise ValueError("parent/input contract")
    parent_outputs = list((parent / "out").glob("*.fcs"))
    if len(parent_outputs) != 1:
        raise ValueError("expected one parent output FCS")
    parent_out, parent_out_names = read_fcs(parent_outputs[0])
    parent_labels = np.rint(parent_out[:, -1]).astype(np.int32)
    if (
        parent_out.shape != (a.n_events, a.n_markers + 1)
        or parent_out_names[:-1] != names
        or not np.array_equal(parent_out[:, :-1], matrix)
    ):
        raise ValueError("parent output contract")
    parent_jar = Path(
    ...
    contract = {
        ...
        "java_util_random_methodrefs": sorted({r for r in refs if r[0] == "java/util/Random"}),
    ...
    truth_raw = pd.read_csv(data, sep="\t" if a.separator == "tab" else ",", usecols=["label"])[
        "label"
    ]
    if a.label_mode == "numeric_1_to_n":
        numeric = pd.to_numeric(truth_raw, errors="coerce")
        ev = (numeric.between(1, a.n_populations) & (numeric % 1 == 0)).to_numpy()
        truth = np.array([str(int(x)) for x in numeric[ev]])
    else:
        all_truth = truth_raw.astype(str).to_numpy()
        ev = all_truth != "unassigned"
        truth = all_truth[ev]
    if (
        len(truth_raw) != a.n_events
        or int(ev.sum()) != a.expected_evaluable
        or np.unique(truth).size != a.n_populations
    ):
    ...
    def one(rep: int) -> dict:
        ...
        try:
            r = subprocess.run(
                cmd, cwd=d, capture_output=True, text=True, timeout=a.timeout_seconds
            )
            timed = False
        except subprocess.TimeoutExpired as e:
            r = None
            timed = True
            (d / "xshift_stdout.log").write_text(e.stdout or "", encoding="utf-8")
            (d / "xshift_stderr.log").write_text(e.stderr or "", encoding="utf-8")
        runtime = time.perf_counter() - start
        if r is not None:
            (d / "xshift_stdout.log").write_text(r.stdout, encoding="utf-8")
            (d / "xshift_stderr.log").write_text(r.stderr, encoding="utf-8")
            code = r.returncode
        else:
            code = None
        cand = list((d / "out").glob("*.fcs")) if (d / "out").is_dir() else []
        ok = False
        detail = ""
        nc = None
        lh = None
    ...
    with ThreadPoolExecutor(max_workers=a.workers) as pool:
        ...
        for f in as_completed(fs):
            ...
            records.append(rec)
            print(
                f"repeat={rec['repeat']:03d} pass={rec['all_checks_passed']} runtime={rec['runtime_seconds']:.2f}s",
                flush=True,
            )
    ...
    for rec in records:
        ...
        parts.append(lab)
        met, pf, _, _, _ = evaluate_multiclass(truth, lab[ev], lab)
        rows.append(
            {
                "repeat": rep,
    ...
    pd.concat(pops, ignore_index=True).to_csv(out / "population_level_metrics.csv", index=False)
    pd.DataFrame(parent_comp).to_csv(out / "parent_partition_comparison.csv", index=False)
    pairs = []
    ...
    pair.to_csv(out / "pairwise_partition_ari.csv", index=False)
    summary = []
    metric_names = (
        "runtime_seconds",
    ...
    ck(
        "population_metrics",
        sum(len(x) for x in pops) == success * a.n_populations,
        f"rows={sum(len(x) for x in pops)}",
    )
    ck("pairwise_count", len(pair) == success * (success - 1) // 2, f"rows={len(pair)}")
    ck(
    ...
    ck("labels_valid", len(rf) == 30 and rf.n_predicted_clusters_all_events.ge(1).all(), "valid")
    ck(
    ...
    pv = pair.partition_ari.to_numpy(float)
    manifest = {
        ...
        "helper_script_sha256": sha256(helper),
        ...
        "parent_output_fcs": str(parent_outputs[0]),
        "parent_output_fcs_sha256": sha256(parent_outputs[0]),
        ...
        "n_evaluable": a.expected_evaluable,
        "n_populations": a.n_populations,
        "workers": a.workers,
    ...
    (out / "scientific_summary.md").write_text(
        f"# {a.experiment_id} Thirty native X-shift repeats on {a.dataset} at fixed K=20\n\nSuccessful runs: {success}/30; unique all-event partition hashes: {unique}; mean/range of the 435 pairwise ARI values={pv.mean():.6f}/[{pv.min():.6f},{pv.max():.6f}]; mean/range of macro F1={rf.macro_f1.mean():.6f}/[{rf.macro_f1.min():.6f},{rf.macro_f1.max():.6f}]. Run numbers denote native repeats, not seeds.\n",
        encoding="utf-8",
    )
